...
#' \item \code{flag_table} `build_adsl()` only. flag table is passed from `spec$flag_table` slot
#' }
#' 
#' @details 
...
#' 
#' \code{values_fn} is passed to \code{pivot_wider()}. 
#' The default is \code{function(x) {ifelse(all(is.numeric(x)), mean(x, na.rm = TRUE), na.omit(x)[1])}}
#'
#' @section Authors: 
