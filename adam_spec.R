#' @title adam spec
#' 
#' \code{adam_spec()} is a wrapper for the \code{adam_*_spec()} functions.
#' It creates a list of specifications on how to extract and process data from adam data sets in a given location. 
#' The resulting list can be passed to \code{adam_prep()}, where the data sets are combined into a single wide format data set.
#'
...
#'  For those, specifications may be created manually using the appropriate \code{adam_*_spec()} function and appended to the specification list created by \code{adam_*_spec()}. 
#'
...
#'
#' Please refer to the documentations of the \code{adam_*_spec()} functions for full details.
#'
...
#' 
#' @seealso \code{\link{adam_adsl_spec()}}, \code{\link{adam_bds_spec()}}
#'
#' @usage 
...
adam_spec <- function(
 ...
 doms      <- str_split( all_files, '/|\\\\')  %>%  
  ...
  if ( any(str_detect(import_files, type_adsl)) ){
    ...
    spec <- spec %>% 
      append(
        map(files_adsl, ~adam_adsl_spec(file = .x, filter = filter))
      )
  ...
  if (any(str_detect(import_files, type_bds))){
    ...
    spec <- spec %>% 
      append(
        map(files_bds, ~adam_bds_spec(file = .x, filter = filter))
      )
  ...
  if(length(files_ignored) >0 ){
    ...
    '\nYou can use the adam_*_spec() functions as appropriate.')
    )
  }
