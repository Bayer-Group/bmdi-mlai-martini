...
The package was developed in the clinical context which is reflected in the default settings and specifics of both main and helper functions. This vignette is solely focused on the standard clinical setting. 
...
The package comes with a number of example data sets from raw sas data sets to read in, to data objects representing intermediate steps (`martini_spec`, `martini_build`) as well as the final output object (`martini_ml`) for further use in the `modtune` and/or `report` modules of the pipeline.
...
In case a particular domain required for your analysis but not included in the current list, please get in touch with the package maintainers 
