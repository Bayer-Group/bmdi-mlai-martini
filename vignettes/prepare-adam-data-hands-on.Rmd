...
would yield a empty data set. 

Common filters may be based on visit or treatment information, as well as flags indicating analysis sets.
...
You may use the `add_bds` argument of `adam_spec()` to force them being treated as bds data sets. For these added data sets, it is particularly important to check the automated column selection (in the resulting spec object) and adjust as needed using `adjust_spec()`.
