...
`adam_spec()` automatically selects a `label` column in _occds_ data sets (e.g. _admh_), that controls the categorization of the occurrences and defines the variables that are created by the `build()` function. If you need a more or less detailed categorization, you need to change the `label` entry.
